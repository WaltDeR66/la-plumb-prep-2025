                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      ...
                      <Card className="border-2 border-blue-200 bg-blue-50 h-full">
                        <CardHeader>
                          ...
                          <CardDescription>
                            Generate comprehensive study plans using AI for any section
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                          <AIStudyPlanGenerator courseId={selectedCourse} />
                        </CardContent>
                      </Card>
...
function AIStudyPlanGenerator({ courseId }: { courseId: string }) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  ...
  const generateStudyPlan = async () => {
    if (!courseId || !sectionNumber) {
      toast({
        ...
        description: "Please select a course and enter a section number",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `AI created a comprehensive ${duration}-minute study plan for Section ${sectionNumber}`,
        variant: "default"
      });
    } catch (error: any) {
      ...
      toast({
        ...
        description: error.message || "Failed to generate study plan with AI",
        variant: "destructive"
      });
    } finally {
  ...
  const saveStudyPlan = async () => {
    ...
    try {
      const studyPlanData = {
        courseId,
        title: generatedPlan.studyPlan.title,
        content: generatedPlan.studyPlan.content,
        type: "study-plan",
        chapter: 1, // Default chapter
        section: sectionNumber,
        difficulty: "medium",
        estimatedDuration: generatedPlan.studyPlan.estimatedDuration,
        isActive: true,
        sortOrder: 0
      };

      await apiRequest("POST", "/api/admin/study-plans/bulk-import", {
        courseId,
      ...
      toast({
        ...
        description: `AI-generated study plan for Section ${sectionNumber} has been added to the course`,
        variant: "default"
      });
      ...
      setSectionNumber("");
      setGeneratedPlan(null);
      queryClient.invalidateQueries({ queryKey: ["/api/admin/course-content", courseId] });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/course-content", courseId, "stats"] });
    } catch (error: any) {
      toast({
        title: "Save failed",
  ...
  if (!courseId) {
    return (
      <div className="text-center py-4 text-muted-foreground">
        <p>Select a course to generate AI study plans</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <Button
        ...
        disabled={isGenerating || !sectionNumber}
        className="w-full"
        data-testid="button-generate-study-plan"
      >
        {isGenerating ? (
          <>
      ...
      {generatedPlan && (
        <div className="mt-4 space-y-3" data-testid="generated-plan-preview">
          ...
          <div className="max-h-48 overflow-y-auto p-3 bg-gray-50 border rounded-lg">
            <pre className="text-sm whitespace-pre-wrap">
              {generatedPlan.studyPlan.content}
            </pre>
          </div>

          <Button onClick={saveStudyPlan} className="w-full" variant="outline" data-testid="button-save-study-plan">
            <Plus className="h-4 w-4 mr-2" />
            Save Study Plan to Course
          </Button>
        </div>
      )}
    </div>
